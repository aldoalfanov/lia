...
<html>
<head>
  <title>Toko Online Home Musik Gitar</title>
      ...
      <div class="table-responsive">          
        <table class="table">
          ...
          <?php 
            ...
            $queryKeranjang = mysqli_query($conn, "SELECT * FROM tabel_trolly WHERE idPelangan='$_SESSION[idPelangan]' ");
            $jumlah = mysqli_num_rows($queryKeranjang);

            if($jumlah > 0){
              $queryTrolly = "SELECT * FROM tabel_trolly WHERE idPelangan='$_SESSION[idPelangan]'";
            $query_trolly = mysqli_query($conn, $queryTrolly);
            while($arrayTrolly = mysqli_fetch_array($query_trolly)){
              $queryProduk = mysqli_query($conn, "SELECT * FROM tabel_gitar WHERE idGitar='$arrayTrolly[idGitar]'");
              ...
              echo '
                <tr>
                  ...
                  <td>
                    ...
                    <h5><strong>Total</strong><span class="titik-total">: Rp  '.number_format($arrayTrolly['harga'], 0, ',', '.').'</span></h5>
                  </td>
            ...
            <tr id="total-bayar">
              <?php 
              $queryTotalBayar = mysqli_query($conn, "SELECT SUM(harga) FROM tabel_trolly WHERE idPelangan='$_SESSION[idPelangan]'");
...
if($belumAda==0){
  ...
  <a href="proses/bayar.php?idPelangan='.$_SESSION['idPelangan'].'&&total='.$arrayTotal[0].'"><button type="button" class="btn btn-success"><strong>Beli Sekarang</strong></button></a>
